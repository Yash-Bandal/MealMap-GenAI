            ...
            <Listbox value={value} onChange={onChange}>
                {() => (
                    <div className="relative mt-1">
                        ...
                        <Listbox.Button className="relative w-full cursor-default rounded-2xl bg-white dark:bg-black/30 border border-gray-200 dark:border-white/10 py-3 pl-4 pr-10 text-left shadow-sm focus:outline-none focus:ring-2 focus:ring-[#FF5E57]/30 sm:text-sm">
                            <span className="block truncate text-gray-800 dark:text-gray-100">{value || 'Select'}</span>
                            <span className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2">
                                <ChevronUpDownIcon className="h-5 w-5 text-gray-400" aria-hidden="true" />
                            </span>
                        ...
                        {/* <Listbox.Options className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white dark:bg-gray-800 py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm transition-all duration-200 ease-in-out"> */}
                        <Listbox.Options className="absolute z-10 mt-2 max-h-60 w-full overflow-auto rounded-2xl bg-white dark:bg-[#0c0d0f] py-1 text-base shadow-lg ring-1 ring-black/10 dark:ring-white/10 outline-none sm:text-sm transition-all duration-200 ease-in-out">
                            {options.map((opt, idx) => (
                                <Listbox.Option
                                    key={idx}
                                    className={({ active }) =>
                                        `relative cursor-pointer select-none py-2.5 pl-10 pr-4 transition-colors duration-150 ease-in-out ${active ? 'bg-[#FF5E57]/10 text-gray-900 dark:bg-white/10 dark:text-white' : 'text-gray-900 dark:text-gray-100'
                                        }`
                                    }
                                    value={opt}
                                >
                                    {({ selected }) => (
                                        <>
                                            ...
                                            {selected && (
                                                <span className="absolute inset-y-0 left-0 flex items-center pl-3 text-[#FF5E57] dark:text-[#FF5E57]">
                                                    <CheckIcon className="h-5 w-5" aria-hidden="true" />
                                                </span>
                                            )}
                                        </>
